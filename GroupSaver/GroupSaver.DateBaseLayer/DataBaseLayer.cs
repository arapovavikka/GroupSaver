...
namespace GroupSaver.DataBase
{
    public class DataBaseLayer
    {
        ...
        public Person GetPersonByVkId(int vkId)
        {
            try
            {
                if (HavePersonVkId(vkId))
                {
                    using (var connection = new SQLiteConnection(System.IO.Path.Combine(_folder, _personTableName)))
                    {
                        var persons = connection.Table<Person>().Where(i => i.VkId == vkId);
                        if (persons.Any())
                        {
                            return new Person()
                            {
                                FirstName = persons.ElementAt(0).FirstName,
                                LastName = persons.ElementAt(0).LastName,
                                Id = persons.ElementAt(0).Id,
                                VkId = vkId
                            };
                        }
                    }
                }
                throw new ArgumentException("Person with id " + vkId + " doesn't exist");
            }
            catch (Exception ex)
            {
                Trace.TraceError("Datebase " + _personTableName + ": Person wasn't found. " + ex.Message);
        ...
        public bool AddGroupSubscribe(int userId, int groupId)
        {
            try
            {
                if (HaveGroup(groupId) && HavePerson(userId) && !HaveSubscribe(groupId, userId))
                {
                    using (
